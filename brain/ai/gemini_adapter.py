...
class GeminiAdapter:
    ...
    def tool_result_to_gemini(self, tool_result):
        ...
        if self._types is not None:
            kwargs = {"name": tool_result.get("tool") or "unknown", "response": response}
            if tool_result.get("call_id"):
                kwargs["id"] = tool_result.get("call_id")
            function_response = self._types.FunctionResponse(**kwargs)
            function_response_part = self._types.Part(function_response=function_response)
